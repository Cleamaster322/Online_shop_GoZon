...
from .models import User, Category, Product, CartItem, Delivery, ProductImage, DeliveryPoint
from .serializers import UserSerializer, CategorySerializer, ProductSerializer, CartItemSerializer, DeliverySerializer, \
    ProductImageSerializer, DeliveryPointSerializer
...
# --- DELIVERY POINT FUNCTIONS ---
@api_view(['GET'])
@permission_classes([AllowAny])
def get_all_delivery_points(request):
    try:
        points = DeliveryPoint.objects.all()
        serializer = DeliveryPointSerializer(points, many=True)
        return Response(serializer.data)
    except Exception as e:
        return Response({'detail': 'Ошибка сервера', 'error': str(e)},
                        status=status.HTTP_500_INTERNAL_SERVER_ERROR)


@api_view(['GET'])
@permission_classes([AllowAny])
def get_delivery_point(request, pk):
    try:
        point = DeliveryPoint.objects.get(pk=pk)
        serializer = DeliveryPointSerializer(point)
        return Response(serializer.data)
    except DeliveryPoint.DoesNotExist:
        return Response({'detail': 'Пункт доставки не найден'},
...
@api_view(['POST'])
@permission_classes([AllowAny])
def post_delivery_point(request):
    try:
...
@api_view(['PUT', 'PATCH'])
@permission_classes([AllowAny])
def update_delivery_point(request, pk):
    try:
        point = DeliveryPoint.objects.get(pk=pk)
        serializer = DeliveryPointSerializer(point, data=request.data, partial=(request.method == 'PATCH'))
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
@api_view(['DELETE'])
@permission_classes([AllowAny])
def delete_delivery_point(request, pk):
    try:
        point = DeliveryPoint.objects.get(pk=pk)
        point.delete()
        return Response({'detail': 'Пункт доставки удалён'},
